...
void Task01_Entry(void *argument);
void Task02_Entry(void *argument);
...
int main(void)
{
  ...
  /* Create the thread(s) */
  /* creation of Task01 */
  Task01Handle = osThreadNew(Task01_Entry, NULL, &Task01_attributes);

  /* creation of Task02 */
  Task02Handle = osThreadNew(Task02_Entry, NULL, &Task02_attributes);
...
//		portYIELD_FROM_ISR( xHigherPriorityTaskWoken ); //context switch immediately if HigherPrio
...
void Task01_Entry(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  HAL_UART_Transmit(&huart2, (uint8_t *)"Running Task 1\r\n",16, 10);
	  osDelay(3000);
  }
  /* USER CODE END 5 */
}
...
void Task02_Entry(void *argument)
{
  ...
  {
	  osSemaphoreAcquire(BinarySem01Handle, osWaitForever);
	  HAL_UART_Transmit(&huart2, (uint8_t *)"Running Task 2\r\n",16, 10);
	  osDelay(1000);
  }
